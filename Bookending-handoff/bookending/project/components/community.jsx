// Community ribbon — "authors helping authors" running through the dashboard.
    ...
    <section style={{padding:'40px 48px 48px', background:'var(--paper)', color:'var(--ink)'}}>
      <div style={{display:'flex', alignItems:'flex-end', justifyContent:'space-between', marginBottom:28, gap:24}}>
        <div>
          ...
          <h2 className="serif" style={{
            ...
            fontSize:46, lineHeight:1, margin:0, letterSpacing:'-0.015em', color:'var(--ink)',
          }}>Authors at your shoulder.</h2>
          <p className="serif" style={{
            fontFamily:'var(--serif)', fontSize:16, color:'#3a3530', margin:'10px 0 0',
        ...
        <Btn tone="bare" icon={<IconArrow size={14}/>}>
          <span style={{color:'var(--ink)'}}>Open the common room</span>
        </Btn>
      </div>
      ...
      <div style={{display:'grid', gridTemplateColumns:'1fr 1fr 1fr 1fr', gap:0, borderTop:'1px solid #00000020'}}>
        {[
          ...
          <article key={i} style={{
            padding:'24px 22px', borderRight: i<3 ? '1px solid #00000020' : 'none',
            display:'flex', flexDirection:'column', gap:12
          }}>
            <div className="mono" style={{fontSize:10, letterSpacing:'0.18em', color:'#00000066'}}>
              {c.kind}
            </div>
            <h3 className="serif" style={{
              fontFamily:'var(--serif)', fontWeight:500, fontSize:19, lineHeight:1.22,
              margin:0, color:'var(--ink)', textWrap:'balance', letterSpacing:'-0.005em'
            }}>{c.head}</h3>
            <p className="serif" style={{
              fontFamily:'var(--serif)', fontStyle:'italic', fontSize:14, lineHeight:1.45,
              margin:0, color:'#3a3530', textWrap:'pretty', flex:1
            }}>{c.body}</p>
            <div style={{
              borderTop:'1px solid #00000020', paddingTop:10, display:'flex', justifyContent:'space-between', alignItems:'center'
            }}>
      ...
      {/* Karma footer */}
      <div style={{
        marginTop:28, padding:'18px 22px', background:'var(--ink)', color:'var(--paper)',
