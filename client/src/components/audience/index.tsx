...
const TOP_READERS = [
  { n: 'Aoife Brennan',  i: 'AB', t: 'gold'   as const, last: 'Replied to every letter for 11 issues',     stat: '11/11' },
  { n: 'Wendell Park',   i: 'WP', t: 'accent' as const, last: 'Pre-ordered all three editions',             stat: '$92' },
  { n: 'Sade Adebayo',   i: 'SA', t: 'paper'  as const, last: 'Brought 4 friends from her bookclub',        stat: '+4' },
  { n: 'Joon Park',      i: 'JP', t: 'muted'  as const, last: 'Wrote a 1,400-word reply to issue 45',      stat: '1.4k' },
  { n: 'Beatriz Costa',  i: 'BC', t: 'ink'    as const, last: 'Bought signed copy + hosted reading',        stat: '★' },
];

const REPLY_SNIPPETS = ['"This made me…"', '"I cried at—"', '"Will there—"', '"My mother—"', '"The lighthouse—"', '"Please don\'t—"'];
    ...
    <section style={{ padding: '40px 48px', borderBottom: '1px solid var(--rule)' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1.1fr', gap: 0, border: '1px solid var(--rule)' }}>
        ...
        <div style={{ padding: 28, borderRight: '1px solid var(--rule)' }}>
          ...
          <h3 className="serif" style={{ fontStyle: 'italic', fontWeight: 400, fontSize: 26, lineHeight: 1.18, margin: '0 0 18px', letterSpacing: '-0.005em' }}>
            On finishing the second draft — and the small grief of it
          </h3>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14, marginBottom: 18 }}>
            {[['Open rate','62.4%','+8.1pt vs avg'],['Replies','148','your highest yet'],['Click rate','24.1%','linked to shop'],['Unsubs','9','0.3%']].map(([l,v,sub]) => (
          ...
          <div style={{ borderTop: '1px solid var(--rule)', paddingTop: 14 }}>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
              {REPLY_SNIPPETS.map(t => (
                <span key={t} className="serif" style={{ fontStyle: 'italic', fontSize: 13, padding: '4px 10px', border: '1px solid var(--rule)', whiteSpace: 'nowrap' }}>{t}</span>
              ))}
            </div>
          </div>
        </div>
        ...
        <div style={{ padding: 28 }}>
          ...
          {TOP_READERS.map((p, i) => (
            <div key={p.n} style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '12px 0', borderBottom: i < 4 ? '1px dashed var(--rule)' : 'none' }}>
              <Avatar initials={p.i} tone={p.t} size={32} />
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontSize: 13, fontWeight: 600 }}>{p.n}</div>
                <div className="serif" style={{ fontStyle: 'italic', fontSize: 12.5, color: 'var(--muted)', marginTop: 2 }}>{p.last}</div>
              </div>
              <div className="mono" style={{ fontSize: 11, letterSpacing: '0.06em', textAlign: 'right', minWidth: 36 }}>{p.stat}</div>
            </div>
          ))}
        </div>
      </div>
