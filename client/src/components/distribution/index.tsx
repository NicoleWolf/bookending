import { SectionHead, Pill, Btn, Spark } from '../atoms';
import { IconArrow, IconMore } from '../icons';

const CHANNELS = [
  { name: 'Amazon KDP',       kind: 'Print + eBook',      status: 'live',    sku: 'B0CTQ4Z9R8',       royalty: '60%', moved: 412, color: 'good'    as const },
  { name: 'IngramSpark',      kind: 'Print · global',     status: 'live',    sku: '978-1-7398..',     royalty: '45%', moved: 184, color: 'good'    as const },
  { name: 'Apple Books',      kind: 'eBook',              status: 'live',    sku: '1452119',          royalty: '70%', moved:  88, color: 'good'    as const },
  { name: 'Kobo Writing Life', kind: 'eBook',             status: 'live',    sku: '9781739..',        royalty: '70%', moved:  51, color: 'good'    as const },
  { name: 'Bookshop.org',     kind: 'Print · indie',      status: 'pending', sku: 'awaiting ISBN map', royalty: '30%', moved:   0, color: 'neutral' as const },
  { name: 'Libro.fm',         kind: 'Audio',              status: 'draft',   sku: 'narrator unbooked', royalty: '—',  moved:   0, color: 'danger'  as const },
];
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1.7fr 1fr', gap: 32 }}>
        ...
        <div style={{ border: '1px solid var(--rule)' }}>
          <div style={{
            ...
            {['Channel', 'Format', 'Status', 'Royalty', 'Moved', '·'].map((h, i) => (
              <div key={h} className="label" style={i === 5 ? { textAlign: 'right' } : {}}>{h}</div>
            ))}
          </div>
          {CHANNELS.map((c, i) => (
            <div key={c.name} style={{
              ...
              <Pill tone={c.color}>{c.status}</Pill>
        ...
        <div style={{ border: '1px solid var(--rule)', padding: 24 }}>
          ...
          <div style={{ display: 'flex', gap: 14, marginBottom: 18 }}>
            <div style={{ flex: 1, aspectRatio: '2/3', background: '#1a1612', position: 'relative', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', padding: '14px 12px' }}>
              ...
              <div>
                <div className="serif" style={{ fontStyle: 'italic', fontSize: 14, color: '#e8d9b8', lineHeight: 1.05 }}>The<br />Lantern<br />Keeper's<br />Daughter</div>
          ...
          <div style={{ borderTop: '1px solid var(--rule)', paddingTop: 14, display: 'flex', flexDirection: 'column', gap: 10 }}>
            {[['Trim','6 × 9 in'],['Paper','55# cream'],['Cover','Matte'],['Spine','0.81 in'],['Extent','322 pp']].map(([k, v]) => (
